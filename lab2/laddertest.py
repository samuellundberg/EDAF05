...
class Node(object):
    ...
    def __init__(self, word, path_to, letters, root_path):
        self.word = word
...
def make_node(word, path_to, letters, root_path):
    node = Node(word, path_to, letters, root_path)
    return node
    ...
    while True:
        for l in layer:
            ...
            for ll in l.path_to:
                temp = rp.copy()
                temp.append(cc)
              #  print('ny = ', ll.root_path)
                if ll.word == word:
#                    print(ll.root_path)
                    ll.root_path = temp.copy()
                    return word_path(root, ll.root_path)    # path.append(word)
...
def word_path(root, list):
    r = root
    count = 1
    words = [root.word]
    while count < len(list):
        new_r = r.path_to[list[count]]
        words.append(new_r.word)
        r = new_r
        count += 1
    return words
